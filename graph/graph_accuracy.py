...
df = pd.read_csv("rapport_federated_learning 11.csv")
...
for h in unique_h:
    ...
    for _, row in round_data.iterrows():
        ...
        current_map = row['mAP50']  # Using mAP50 as the accuracy metric
        t_debut = row['Debut_Round_Sec']
        t_fin_calcul = row['Fin_Entrainement_Sec']
        
        # Check if it's the very first round
        if h == unique_h[0]:
            # For Round 1, start plotting directly from the end of its training
            extended_rows.append({'Time_Sec': t_fin_calcul, 'mAP50': current_map, 'Device_ID': device})
            extended_rows.append({'Time_Sec': fin_round_global, 'mAP50': current_map, 'Device_ID': device})
        else:
            # For subsequent rounds, look up the performance from the previous round
            prev_map = df[(df['Device_ID'] == device) & (df['Horodatage'] < h)]['mAP50'].iloc[-1]
            
            # Point A: Round start (takes the previous actual accuracy value)
            extended_rows.append({'Time_Sec': t_debut, 'mAP50': prev_map, 'Device_ID': device})
            
            # Point B: Training end (accuracy climbs to current value)
            extended_rows.append({'Time_Sec': t_fin_calcul, 'mAP50': current_map, 'Device_ID': device})
            
            # Point C: Global round end (accuracy plateaus horizontally during synchronization wait)
            extended_rows.append({'Time_Sec': fin_round_global, 'mAP50': current_map, 'Device_ID': device})
...
for i, h in enumerate(unique_h):
    ...
    ax.text(fin_sec - 15, ax.get_ylim()[1] - (ax.get_ylim()[1]*0.02), f"Round {i+1}", 
            rotation=90, va='top', ha='right', color='blue', fontsize=9, alpha=0.6)

# Graph styling and English translations
ax.set_title("Federated Learning Training Accuracy (mAP50) & Synchronization Idle Times", fontsize=13, fontweight='bold', pad=15)
